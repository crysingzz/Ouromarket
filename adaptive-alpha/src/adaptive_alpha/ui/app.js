...
function renderAutonomous(){
 ...
 const packets=(data.knowledge.packets||[]).map(p=>`<article class="strategy-card">${badge(p.status)}<h3>${escape(p.department==='replication'?'Воспроизведение стратегии':'Поиск новой гипотезы')}</h3><p>${escape(p.query)}</p><p>Источники: ${escape(p.requested_sources.join(', '))}</p><p>Цитируемых фрагментов: ${p.passages.length}</p><p>Пробелы: ${escape(p.gaps.join('; ')||'не зафиксированы')}</p><small>${escape(short(p.id))}</small></article>`).join('');
 const publications=data.knowledge.evidence.map(e=>`<div class="detail-row"><span>${escape(e.title)}<small>${escape(e.provider)} · ${escape(e.published)} · ${escape(e.content_level||'metadata')}</small></span><span>${escape(short(e.id))}</span></div>`).join('');
 $('knowledge-list').innerHTML=packets+publications||empty('No retrieved evidence.','Campaigns retain publication metadata, bounded passages and their cited relationships.');
}
