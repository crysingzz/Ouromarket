...
Separate research departments, 2026-09-17: **455 tests passed; 100.00% Adaptive Alpha statement coverage (5277/5277)**. Three focused acceptance tests cover fixed replication/novel queue ownership, rejection of legacy unbound work by production workers, immutable campaign policy and budget identity, department-scoped memory, exact memory snapshots across recovery, packet/result-policy mismatches, independent readiness, UI and two-worker Compose topology. Ruff, Pyrefly, mypy, dependency audit and all 24 strict OpenSpec items pass. Controlled generators are used; this does not configure model execution, verify claim truth or grant capital authority.

Research claims and mechanism identity, 2026-09-17: **452 tests passed; 100.00% Adaptive Alpha statement coverage (5151/5151)**. Four focused tests cover deterministic claim and mechanism identity, exact packet and citation binding, supports/contradicts graph retention, anchor versus semantic status, malformed and inconsistent contracts, Unicode/case/punctuation normalization, exact and family matches, unbounded immutable candidate-mechanism lookup, novel-department duplicate refusal, replication lineage, authenticated API and UI. Ruff, Pyrefly, mypy, dependency audit and all 23 strict OpenSpec items pass. Controlled researchers and transports are used; this does not independently verify claim truth, formulas or scientific novelty.

Bounded arXiv HTML acquisition, 2026-09-15: **448 tests passed; 100.00% Adaptive Alpha statement coverage (5047/5047)**. Four focused tests cover fixed-origin derivation, inert bounded text, provenance and license retention, invalid identities, HTTP failure, non-HTML content, oversized and insufficient responses, abstract fallback, partial packet status, campaign persistence, authenticated API and UI. Ruff, Pyrefly, mypy, dependency audit and all 22 strict OpenSpec items pass. All network behavior is tested through fixed mock transports; this verification does not claim a live arXiv fetch, formula interpretation or scientific replication.

Evidence packet binding, 2026-09-14: **444 tests passed; 100.00% Adaptive Alpha statement coverage (4977/4977)**. Replication and novel campaigns now retain content-addressed search scope, provider health, deterministic source versions, bounded extractive passages and explicit gaps. Real researcher output must cite exact passages and acknowledge every gap; WorkOrder registration revalidates the packet and retained sources before Ouroboros receives it. API/UI visibility and hostile document instructions are tested. Ruff, Pyrefly, mypy, dependency audit and all 21 strict OpenSpec items pass. Current public connectors still retrieve metadata and abstracts, so this does not claim full-text replication or verified scientific novelty.

Engineering tool qualification, 2026-09-14: **437 tests passed; 100.00% Adaptive Alpha statement coverage (4788/4788)**. Twelve focused tests cover deterministic and idempotent baseline/trial plans, three independent task bindings, server-owned qualification, wins/ties/regressions, reviewed source/runtime provenance, operator-only adoption and revocation, harness run evidence, dependency protection, safe supersession, immutable WorkOrder tool snapshots, revoked-before-start refusal, API authority, audit integrity and UI visibility. Ruff, Pyrefly, mypy and all strict OpenSpec items pass. Tests use controlled strategy outputs; no model-generated tool, OpenAI call or gVisor host was admitted, and every tool record remains ineligible for capital, evaluator, risk or broker authority.

Reviewed harness queue, 2026-09-13: **425 tests passed; 100.00% Adaptive Alpha statement coverage (4471/4471)**. The 15 focused tests verify operator-only admission and cancellation, exact artifact/review/runtime/request/schema bindings, idempotency conflicts, lease bounds and resume, stale-owner fencing, cancellation propagation, quarantined-runner refusal, output validation, result provenance, audit integrity, UI visibility and Compose isolation. Ruff, Pyrefly and mypy pass. The separate worker is the only application service with the Docker daemon socket mounted and it has only the database network; the public API and agents do not receive runner authority. This controlled verification does not execute model-generated source, accept a gVisor host, enable automatic tool adoption or grant capital authority.
