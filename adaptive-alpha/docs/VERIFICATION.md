...
Engineering attempt journal, 2026-09-12: **381 tests passed; 100.00% Adaptive Alpha statement coverage (3811/3811)**. Every dispatch is recorded before the runtime call with its requested token/time budget; append-only events retain running, contract-validation and terminal state. A deterministic task identity prevents duplicate model work after an ambiguous create response, and a retry resumes only a retained task bound to the same workspace. Campaign ownership is checked during polling; cancellation or lease loss propagates to the upstream cancel endpoint and ends the journal entry as CANCELLED. Ruff, Pyrefly, mypy and all 20 strict OpenSpec items pass. Tests cover success, contract failure, invalid transitions, identity collision, cancellation, credential-free worker status and exception-message redaction. This journal does not yet provide an independently leased task queue or reconstruction of an interrupted campaign from its retained WorkOrder.
